...
/************************************************/
	...
	/* Message subject */
	$your_subject = "J-forms: Password recovery";

	/* Define your data to access MySQL database */
	define("REC_USER", "username");					// your username
	define("REC_SERVER", "host");					// your host
	define("REC_PASSWORD", "password");				// your password
	define("REC_DATABASE", "database");				// your database

	/* Your database table goes here */
	$mysql_table = "recovery";
...
/************************************************/
	...
	$token	 = (isset($_POST["token_recovery"])) ? strip_tags(trim($_POST["token_recovery"]))	: false;
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $email);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $email);
		}
	}
