...
/************************************************/
	...
	define("PARTY_USER", "username");					// your username
	define("PARTY_SERVER", "host");						// your host
	define("PARTY_PASSWORD", "password");				// your password
	define("PARTY_DATABASE", "database");				// your database
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $you_make_it, $guest_quantity, $friends, $message);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $you_make_it, $guest_quantity, $friends, $message);
		}
	}
