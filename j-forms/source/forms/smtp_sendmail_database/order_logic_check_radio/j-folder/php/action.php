...
/************************************************/
	...
	define("ORDER_USER", "username");					// your username
	define("ORDER_SERVER", "host");						// your host
	define("ORDER_PASSWORD", "password");				// your password
	define("ORDER_DATABASE", "database");				// your database
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $cake_size, $filling, $candles, $show_inscription, $inscription, $delivery, $name, $phone, $email, $address);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $cake_size, $filling, $candles, $show_inscription, $inscription, $delivery, $name, $phone, $email, $address);
		}
