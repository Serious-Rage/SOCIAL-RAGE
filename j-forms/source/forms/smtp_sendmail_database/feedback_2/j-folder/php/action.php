...
/************************************************/
	...
	define("FB_USER", "username");					// your username
	define("FB_SERVER", "host");					// your host
	define("FB_PASSWORD", "password");				// your password
	define("FB_DATABASE", "database");				// your database
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $name, $email, $department, $message, $product, $service, $support);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $name, $email, $department, $message, $product, $service, $support);
		}
	}
