...
/************************************************/
	...
	define("COM_USER", "username");					// your username
	define("COM_SERVER", "host");					// your host
	define("COM_PASSWORD", "password");				// your password
	define("COM_DATABASE", "database");				// your database
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $first_name, $last_name, $email, $url, $comments, $notify);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $first_name, $last_name, $email, $url, $comments, $notify);
		}
	}
