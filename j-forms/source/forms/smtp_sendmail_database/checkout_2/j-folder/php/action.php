...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $name, $email, $phone, $country, $city,
									$address, $message, $payment, $card_name, $card_number, $cvv2, $card_month, $card_year);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $name, $email, $phone, $country, $city,
									$address, $message, $payment, $card_name, $card_number, $cvv2, $card_month, $card_year);
		}
	}
...
/* Success message */
/************************************************/
	echo '<div class="success-message unit"><i class="fa fa-check"></i>Your information has been sent</div>';
?>