...
/************************************************/
	...
	define("SER_USER", "username");					// your username
	define("SER_SERVER", "host");					// your host
	define("SER_PASSWORD", "password");				// your password
	define("SER_DATABASE", "database");				// your database
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $name, $company, $email, $phone, $service, $budget, $date_from, $date_to,
										$first_file_name, $second_file_name, $message);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $name, $company, $email, $phone, $service, $budget, $date_from, $date_to,
										$first_file_name, $second_file_name, $message);
		}
	}
