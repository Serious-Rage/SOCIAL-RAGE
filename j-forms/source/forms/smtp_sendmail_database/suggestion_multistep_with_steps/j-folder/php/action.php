...
/************************************************/
	...
	/* Message subject */
	$your_subject = "J-forms: Suggestion form";

	/* Define your data to access MySQL database */
	define("SUG_USER", "username");					// your username
	define("SUG_SERVER", "host");					// your host
	define("SUG_PASSWORD", "password");				// your password
	define("SUG_DATABASE", "database");				// your database

	/* Your database table goes here */
	$mysql_table = "suggestion";
...
/************************************************/
	...
	$token		= (isset($_POST["token_suggestion"]))	? strip_tags(trim($_POST["token_suggestion"]))  : false;
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $first_name, $last_name, $email, $department, $subject, $file_name, $message);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $first_name, $last_name, $email, $department, $subject, $file_name, $message);
		}
	}
