...
/************************************************/
	...
	/* Message subject */
	$your_subject = "J-forms: Subscribe form";

	/* Define your data to access MySQL database */
	define("SUB_USER", "username");					// your username
	define("SUB_SERVER", "host");					// your host
	define("SUB_PASSWORD", "password");				// your password
	define("SUB_DATABASE", "database");				// your database

	/* Your database table goes here */
	$mysql_table = "subscribe";
...
/************************************************/
	...
	$news		 = (isset($_POST["news_group"]))		? subscribeGroup($_POST["news_group"])		  : false;
	$newsletter	 = (isset($_POST["newsletter_group"]))	? subscribeGroup($_POST["newsletter_group"])  : false;
...
/************************************************/
	...
	if ($validate_newsletter){
		$result = validateNewsGroup($newsletter);
		if ($result !== "valid") {
			$error_text[] = $result;
		}
	}
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $email, $news, $newsletter);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $email, $news, $newsletter);
		}
	}
