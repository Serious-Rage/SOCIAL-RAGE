...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $name, $email, $phone, $adults, $children, $date_from, $date_to, $message);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $name, $email, $phone, $adults, $children, $date_from, $date_to, $message);
		}
	}
