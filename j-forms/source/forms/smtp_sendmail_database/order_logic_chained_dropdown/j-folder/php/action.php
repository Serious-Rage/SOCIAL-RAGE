...
/************************************************/
	...
	define("NOTE_USER", "username");					// your username
	define("NOTE_SERVER", "host");						// your host
	define("NOTE_PASSWORD", "password");				// your password
	define("NOTE_DATABASE", "database");				// your database

	/* Your database table goes here */
	$mysql_table = "notebook";
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $notebook, $notebook_model, $notebook_model_action, $software, $courier, $total_service, $name, $phone, $email, $address);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $notebook, $notebook_model, $notebook_model_action, $software, $courier, $total_service, $name, $phone, $email, $address);
		}
