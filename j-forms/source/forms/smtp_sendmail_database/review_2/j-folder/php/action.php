...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $first_name, $last_name, $email, $phone, $message, $product, $service, $delivery, $support);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $first_name, $last_name, $email, $phone, $message, $product, $service, $delivery, $support);
		}
	}
