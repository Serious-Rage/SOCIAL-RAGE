...
/************************************************/
	...
	/* Message subject */
	$your_subject = "J-forms: Error report";

	/* Define your data to access MySQL database */
	define("ER_USER", "username");						// your username
	define("ER_SERVER", "host");						// your host
	define("ER_PASSWORD", "password");					// your password
	define("ER_DATABASE", "database");					// your database

	/* Your database table goes here */
	$mysql_table = "error_report";
...
/************************************************/
	...
	$token	 = (isset($_POST["token_er_report"]))	? strip_tags(trim($_POST["token_er_report"]))	: false;
...
/************************************************/
	if ($duplicate_to_database) {
		...
		if ($mysqli_connect) {
			...
			$row_id = queryMysqli($mysql_table, $message, $email, $file_name);
		}
		/* PDO connection to DB */
		$pdo_connect = false;
		if ($pdo_connect) {
			require dirname(__FILE__)."/pdo.php";
			$row_id = queryPdo($mysql_table, $message, $email, $file_name);
		}
	}
