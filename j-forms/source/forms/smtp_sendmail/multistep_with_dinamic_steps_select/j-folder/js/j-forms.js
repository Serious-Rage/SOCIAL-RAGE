$(document).ready(function(){
	...
	$( '#j-forms' ).validate({
		...
		messages: {
			...
			next_step_select: {
				required: 'Please choose the action'
			},
			airport: {
	...
	if ( $('form.j-multistep').length ) {
		...
		$('form.j-multistep').each( function() {
			...
			var $index_logic_block_select = $('#' + $id + ' .logic-block-select').closest('fieldset').index('fieldset');
			...
			$prev_btn.on('click', function() {
				...
				// processing the buttons
				if ( $('#' + $id + ' fieldset').eq($index_logic_block_select-1).hasClass('active-fieldset') ) {
					$submit_btn.css('display', 'none');
